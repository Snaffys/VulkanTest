...
private:
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        renderPassInfo.clearValueCount = 1; // number of attachments (color, depth, stencil etc.)
    ...
    void createSwapChain() {
        ...
        if (swapChainSupport.capabilities.maxImageCount > 0 && imageCount > swapChainSupport.capabilities.maxImageCount) { // maxImageCount == 0 = no limit
    ...
    bool checkValidationLayerSupport() {
        ...
        if (!unavailableLayers.empty()) {
            std::cout << "Unavailable layers:" << std::endl;
    ...
    void requiredExtensionsSupported(const char** requiredExtensions, uint32_t requiredExtensionCount) {
        ...
        for (uint32_t i = 0; i < requiredExtensionCount; i++) {
            bool requiredExtSupported = false;
            for (uint32_t j = 0; j < supportedExtensionCount; j++) {
                if (strcmp(requiredExtensions[i], supportedExtensions[j].extensionName) == 0){
                    requiredExtSupported = true;
                    break;
                }
            }
            if (!requiredExtSupported) {
                unsupportedExtensions.push_back(requiredExtensions[i]);
            }
        }
    ...
    void createInstance() {
        ...
        requiredExtensionsSupported(extensions.data(), static_cast<uint32_t>(extensions.size()));
